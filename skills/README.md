---
name: skills-index
description: Index of local pi skills in ~/.pi/agent/skills; use only to browse available skill files.
disable-model-invocation: true
tags: [skill, index]
---

# Skills

| Skill                                       | Description                                                                       |
| ------------------------------------------- | --------------------------------------------------------------------------------- |
| [doc-polisher](doc-polisher/SKILL.md)       | Format docs via nvim conform and standardize YAML frontmatter metadata            |
| [feature-builder](feature-builder/SKILL.md) | Build approved docs/plans while maintaining current state, handoff, and ledger    |
| [git-committer](git-committer/SKILL.md)     | Group related changes from a git diff into clean atomic commits                   |
| [handoff-writer](handoff-writer/SKILL.md)   | Update plan-doc handoff state or write standalone handoffs for fresh agents       |
| [pr-drafter](pr-drafter/SKILL.md)           | Draft a PR description from git diff, linked issue, build status, and docs        |
| [pr-responder](pr-responder/SKILL.md)       | Triage and resolve PR review comments — fix, push back, or reply inline           |
| [pr-reviewer](pr-reviewer/SKILL.md)         | Structured GitHub PR review with optional post via MCP or gh CLI                  |
| [readme-updater](readme-updater/SKILL.md)   | Scan repo and update README with setup, install, usage, examples, agentic assets  |
| [rosetta-stoner](rosetta-stoner/SKILL.md)   | Transliterate and translate classical and sacred texts with line-by-line meanings |
| [skill-creator](skill-creator/SKILL.md)     | Create new agent skills in this library                                           |
| [skill-sharpener](skill-sharpener/SKILL.md) | Refine existing skills by reconciling edits into clean SKILL.md                   |
| [tech-specer](tech-specer/SKILL.md)         | Write docs/plans with current state, handoff, implementation plan, and ledger     |
